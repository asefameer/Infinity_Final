...
const EventDetail = () => {
  ...
  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'Event', name: event.title, startDate: event.date,
      ...
      <div className="container mx-auto px-6 -mt-20 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-4">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="aspect-[16/9] rounded-xl bg-muted flex items-center justify-center"><span className="text-muted-foreground text-sm">Map coming soon</span></div>
            {event.lineup && (
              <div>
                <h2 className="font-display font-semibold text-xl mb-4">Lineup</h2>
                <div className="flex flex-wrap gap-3">{event.lineup.map(artist => <span key={artist} className="px-4 py-2 rounded-full border border-border/40 text-sm">{artist}</span>)}</div>
              </div>
            )}
            {event.schedule && (
              <div>
                <h2 className="font-display font-semibold text-xl mb-4">Schedule</h2>
                <div className="space-y-3">{event.schedule.map((s, i) => <div key={i} className="flex gap-4 text-sm"><span className="text-muted-foreground w-20 flex-shrink-0">{s.time}</span><span>{s.activity}</span></div>)}</div>
              </div>
            )}
            {event.faq.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {event.faq.map((f, i) => (
                    <div key={i} className="border border-border/30 rounded-xl">
                      <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between p-4 text-sm font-medium text-left">{f.question}{openFaq === i ? <ChevronUp className="w-4 h-4 flex-shrink-0" /> : <ChevronDown className="w-4 h-4 flex-shrink-0" />}</button>
            ...
            {event.ticketTiers.map(tier => (
              <div key={tier.id} className="p-4 rounded-lg border border-border/20 space-y-2">
                <div className="flex justify-between items-start">
                  <div><h4 className="font-medium text-sm">{tier.name}</h4><p className="text-xs text-muted-foreground">{tier.description}</p></div>
