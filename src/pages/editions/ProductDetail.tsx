...
const ProductDetail = () => {
  ...
  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'Product', name: product.name, description: product.description, image: product.images[0],
    offers: { '@type': 'Offer', price: product.price, priceCurrency: product.currency, availability: product.inStock ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock' },
  };
          ...
          <div className="flex flex-col gap-6">
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                {product.compareAtPrice && <span className="text-lg text-muted-foreground line-through">৳{product.compareAtPrice.toLocaleString()}</span>}
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {v.options.map(opt => (
                    <button key={opt} onClick={() => setSelectedVariants(prev => ({ ...prev, [v.name]: opt }))}
                      className={`px-4 py-2 rounded-full border text-sm transition-colors ${selectedVariants[v.name] === opt ? 'border-primary bg-primary/10 text-foreground' : 'border-border/40 text-muted-foreground hover:border-foreground hover:text-foreground'}`}>{opt}</button>
                  ))}
                </div>
              </div>
            ))}
            <div>
              ...
              <div className="flex items-center gap-3 border border-border/40 rounded-full w-fit">
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center hover:bg-muted/50 rounded-l-full transition-colors"><Minus className="w-4 h-4" /></button>
                <span className="text-sm font-medium w-8 text-center">{quantity}</span>
                <button onClick={() => setQuantity(q => q + 1)} className="w-10 h-10 flex items-center justify-center hover:bg-muted/50 rounded-r-full transition-colors"><Plus className="w-4 h-4" /></button>
              </div>
            </div>
            ...
            {[
              ...
              <div key={section.id} className="border-t border-border/30">
                <button onClick={() => setOpenAccordion(openAccordion === section.id ? null : section.id)} className="w-full flex items-center justify-between py-4 text-sm font-medium">
                  {section.title}
                  {openAccordion === section.id ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </button>
                {openAccordion === section.id && <p className="text-sm text-muted-foreground pb-4 whitespace-pre-line">{section.content}</p>}
              </div>
            ))}
            <div className="flex flex-col gap-3 p-4 rounded-xl bg-muted/30 border border-border/20">
              <div className="flex items-center gap-3 text-sm"><Truck className="w-4 h-4 text-muted-foreground flex-shrink-0" /><span>Free delivery across Bangladesh on orders over ৳5,000</span></div>
              <div className="flex items-center gap-3 text-sm"><RefreshCw className="w-4 h-4 text-muted-foreground flex-shrink-0" /><span>Easy returns within 14 days</span></div>
            </div>
          </div>
        </div>
      </div>
      {related.length > 0 && <ScrollReveal><ProductCarousel title="You May Also Like" products={related} /></ScrollReveal>}
    </>
  );
};
